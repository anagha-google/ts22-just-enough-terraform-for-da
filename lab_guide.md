...
In this section, we will provision networking entities needed for VPC native services/those that support BYO VPC - such as Cloud Dataproc, Cloud Composer, Cloud Dataflow, Vertex AI Workbench. 
 
1. Move the file network.tf as shown below to the Terraform root directory
```
cd ~/ts22-just-enough-terraform-for-da/00-setup/
cp shelf/network.tf .
```
...
3. While its running, in a separate Cloud Shell tab, open the file and read its contents
```
cat ~/ts22-just-enough-terraform-for-da/00-setup/network.tf
```
a) It first creates VPC,
b) And a subnet with Private Google Access
c) It creates a specific firewall rule
d) It creates a reserved IP needed for Vertex AI workbench, for BYO network
e) It creates peers the network with the Google tenant network - again, needed for Vertex AI workbench, for BYO network
 
3. Terraform will incrementally run every .tf file in the root directory and any updates to teh same when an "apply" is issued. It will therefore run the network.tf
4. Observe the output in the other tab
In the end, you should see-
 ```
 Apply complete! Resources: 42 added, 0 changed, 0 destroyed.
 ```
